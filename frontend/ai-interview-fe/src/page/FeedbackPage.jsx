...
export default function FeedbackPage() {
  ...
  const handleDeletePracticeSession = async (practiceId, event) => {
    ...
    if (window.confirm("Are you sure you want to delete this practice session? This action cannot be undone.")) {
      try {
        ...
        toast.success(response.message || "Practice session deleted successfully");
        fetchPracticeSessions();
      } catch (err) {
        console.error("Error deleting practice session:", err);
        toast.error(err.message || "Failed to delete practice session. Please try again.");
      }
    }
  };
                  ...
                  {loadingPractice ? (
                        ...
                        {practiceSessions.map((practice, idx) => {
                          ...
                          return (
                            <div
                              ...
                              onClick={() => navigate(`/feedback/${practice.id}`)}
                            >
                              <button
                                onClick={(e) => handleDeletePracticeSession(practice.id, e)}
                    ...
                    <div className="text-center py-12 mt-4 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
                      ...
                      <p className="text-sm text-gray-500 mb-6">
                        Click "Practice Again" to start your first practice session
                      </p>
                      <button
                        onClick={handlePracticeAgain}
                        className="inline-flex items-center gap-2 px-5 py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-semibold shadow-lg shadow-green-200 hover:shadow-xl transition-all duration-300"
            ...
            >
              <div className='flex items-center gap-3'>
                ...
                <div className='text-left'>
                  ...
                  <p className='text-sm text-gray-500'>
                    {totalQuestions} question{totalQuestions > 1 ? 's' : ''} with detailed feedback
                  </p>
                </div>
              </div>
              <div className='text-gray-400'>
