"""Model loaders: MultitaskJudge, Llama-1B, Qwen-3B, Qwen-4B"""
import time
import logging
import warnings
import torch
...
class Qwen4BModelManager(BaseModelManager):
    """Manager for Qwen-4B model (Qwen 2.5 4B Instruct)"""
    
    def __init__(self):
        self.model = None
        self.tokenizer = None
        self.model_path = QWEN_4B_MODEL_PATH
        self.device = None
        self.use_4bit = QWEN_4B_USE_4BIT
        self.max_seq_length = QWEN_4B_MAX_SEQ_LENGTH
        self.device_map = QWEN_4B_DEVICE_MAP
    
    def load(self) -> None:
        """Load Qwen-4B model and tokenizer"""
        logger.info("="*60)
        logger.info("Loading Qwen-4B Model...")
        logger.info(f"[Qwen-4B] Model path: {self.model_path}")
        logger.info("="*60)
        ...
        try:
            ...
            logger.info("[Qwen-4B] Loading tokenizer...")
            self.tokenizer = AutoTokenizer.from_pretrained(
                str(self.model_path),
                trust_remote_code=True
            )
            ...
            quantization_config = None
            if self.use_4bit and self.device == DEVICE_CUDA:
                logger.info("[Qwen-4B] Using 4-bit quantization")
                quantization_config = BitsAndBytesConfig(
                    load_in_4bit=True,
                    bnb_4bit_compute_dtype=torch.float16,
                    bnb_4bit_use_double_quant=True,
            ...
            logger.info("[Qwen-4B] Loading model weights...")
            self.model = AutoModelForCausalLM.from_pretrained(
                str(self.model_path),
                quantization_config=quantization_config,
                device_map=self.device_map if self.device == DEVICE_CUDA else None,
            ...
            load_time = time.time() - start_time
            logger.info(f"[Qwen-4B] Model loaded in {load_time:.2f}s")
            logger.info(f"[Qwen-4B] Vocab size: {len(self.tokenizer)}")
            logger.info(f"[Qwen-4B] Device: {self.get_device()}")
            
            if self.device == DEVICE_CUDA:
                allocated = torch.cuda.memory_allocated(0) / (1024**3)
                reserved = torch.cuda.memory_reserved(0) / (1024**3)
                logger.info(f"[Qwen-4B] VRAM: {allocated:.2f} GB / {reserved:.2f} GB reserved")
            
        except Exception as e:
            logger.error(f"[Qwen-4B] Failed to load model: {e}")
            raise
    
    def cleanup(self) -> None:
        """Free resources and cleanup"""
        logger.info("[Qwen-4B] Cleaning up...")
        if self.model:
            del self.model
            self.model = None
        if self.tokenizer:
        ...
        if self.device == DEVICE_CUDA:
            ...
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                logger.info("[Qwen-4B] VRAM cache cleared")
    ...
    def _detect_device(self) -> str:
        ...
        if torch.cuda.is_available():
            gpu_name = torch.cuda.get_device_name(0)
            gpu_memory = torch.cuda.get_device_properties(0).total_memory / (1024**3)
            logger.info(f"[Qwen-4B] Detected GPU: {gpu_name}")
            logger.info(f"[Qwen-4B] VRAM: {gpu_memory:.2f} GB")
            return DEVICE_CUDA
        elif hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
            logger.info("[Qwen-4B] Detected Apple Silicon GPU (MPS)")
            return DEVICE_MPS
        else:
            logger.info("[Qwen-4B] No GPU detected, using CPU")
            return DEVICE_CPU
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            "model_name": "Qwen-4B",
            "model_type": "Qwen 2.5 4B Instruct (Decoder-only)",
            "parameters": "4B",
            "device": self.get_device(),
            "vocab_size": len(self.tokenizer) if self.tokenizer else 151936,
            "max_length": self.max_seq_length,
